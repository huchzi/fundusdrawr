create_template <- function(eye_side = "OD", clip, scale_image = 1, inverted = FALSE, show_controls = FALSE) {
  h <- 400 * scale_image
  w <- 400 * scale_image
  center_x <- w / 2
  center_y <- h / 2

  svg <- xml2::xml_new_root(
    "svg",
    xmlns = "http://www.w3.org/2000/svg",
    width = w,
    height = h,
    viewBox = glue::glue("0 0 {w} {h}")
  )

  style <- xml2::xml_add_child(svg, "style")

  xml2::xml_set_text(style, ".grid { fill:none; stroke:black; stroke-width:1 } .border { fill:none; stroke:black; stroke-width:2 } .vessel { fill:none; stroke:#f20d0d; stroke-width:3 } .invert-button { cursor:pointer }")
  transform_attributes <- ifelse(eye_side == "OS",
    glue::glue("translate({w},0) scale(-{scale_image} {scale_image})"),
    glue::glue("scale({scale_image} {scale_image})")
  )

  ora_path <- "M93.472,108.86C97.42,108.86 101.422,110.344 105.654,110.047C110.578,109.701 124.434,99.951 127.092,96.189C130.152,91.855 132.572,86.841 135.255,82.265C135.894,81.176 137.521,76.345 138.495,75.858C141.034,74.588 146.635,76.047 149.424,75.831C152.77,75.573 156.298,73.794 159.338,72.561C163.936,70.697 168.344,67.389 172.41,64.606C173.367,63.951 176.066,60.907 177.251,60.658C179.066,60.276 183.928,62.39 185.748,63.186C189.653,64.894 196.191,66.255 200.257,66.456C206.469,66.763 211.484,66.165 217.361,64.279C217.926,64.098 223.661,62.119 224.361,62.708C225.711,63.845 226.585,66.848 228.344,68.284C235.249,73.925 244.849,76.644 253.64,77.083C257.567,77.28 260.284,75.498 263.656,73.603C264.113,73.346 266.898,71.475 267.52,71.859C267.761,72.007 267.998,75.35 268.09,75.674C268.981,78.797 271.267,85.479 273.846,87.815C279.792,93.198 285.027,91.116 291.516,93.636C291.917,93.791 291.629,96.53 291.638,96.795C291.713,99.084 292.905,103.101 294.285,105.11C296.589,108.463 301.142,112.523 304.679,114.7C306.349,115.727 310.301,116.347 311.507,117.665C312.228,118.453 311.954,122.311 312.314,123.47C313.629,127.706 314.848,132.618 316.93,136.552C319.076,140.607 321.617,143.614 324.937,146.785C326.083,147.88 328.782,149.937 328.977,151.754C329.785,159.299 325.209,166.458 326.387,173.989C327.176,179.03 331.237,183.717 334.208,187.647C334.974,188.661 338.007,191.387 337.929,192.835C337.717,196.749 333.822,200.355 332.339,203.858C330.122,209.092 329.128,214.914 328.123,220.48C326.9,227.251 325.377,235.767 326.591,242.665C327.069,245.382 329.776,248.84 329.422,251.628C328.982,255.09 324.115,258.182 321.92,260.631C318.387,264.573 313.048,270.154 311.054,275.11C309.691,278.499 309.204,284.152 306.816,286.897C300.632,294.005 287.802,293.646 280.345,300.004C276.008,303.702 272.743,309.259 270.315,314.404C269.756,315.586 268.524,321.423 267.057,322.01C265.041,322.816 260.869,321.356 258.723,321.195C252.595,320.734 246.024,320.517 240.1,322.286C236.503,323.36 231.024,324.85 228.303,327.576C225.905,329.977 220.146,337.812 215.737,337.108C209.622,336.132 204.167,327.852 196.417,326.785C188.797,325.736 180.242,326.187 173.54,329.984C172.576,330.53 165.474,336.547 164.044,335.819C162.553,335.06 161.409,328.917 160.748,327.314C158.808,322.613 154.048,316.676 149.724,313.971C141.653,308.921 134.608,309.397 125.421,309.397C125.263,309.397 114.611,311.529 114.483,310.717C112.825,300.131 109.444,288.394 99.114,283.011C95.719,281.242 92.214,279.961 88.321,279.961C87.845,279.961 83.847,279.745 83.536,279.252C82.536,277.67 81.987,272.835 81.603,270.864C80.197,263.646 79.586,255.371 76.824,248.521C75.049,244.12 67.193,243.241 65.725,239.199C64.371,235.472 67.088,230.272 67.831,226.683C69.381,219.188 71.206,206.729 66.52,199.935C65.611,198.617 59.886,194.267 59.732,193.103C59.479,191.176 62.479,187.393 63.276,185.596C65.864,179.762 68.555,174.502 70.233,168.276C71.122,164.978 71.554,159.568 70.878,156.201C70.707,155.347 68.783,153.514 69.116,152.782C71.068,148.482 76.249,145.626 79.481,142.402C82.167,139.723 84.11,135.944 86.144,132.721C91.494,124.239 89.734,115.302 92.506,106.371"

  defs <- xml2::xml_add_child(svg, "defs")
  clip_def <- xml2::xml_add_child(defs, "clipPath", id = "oraClip")

  transform_clip <- ifelse(eye_side == "OS",
    glue::glue("translate(400,0) scale(-1 1)"),
    glue::glue("scale(1 1)")
  )
  xml2::xml_add_child(
    clip_def,
    "path",
    d = ora_path,
    transform = transform_clip
  )

  xml2::xml_add_child(svg,
    "rect",
    width = "100%",
    height = "100%",
    fill = "white"
  )

  if (show_controls) {
    control <- xml2::xml_add_child(
      svg,
      "g",
      id = "invert-control",
      class = "invert-button",
      onclick = "if (window.Shiny) Shiny.setInputValue('toggle_inverted', Math.random(), {priority: 'event'})"
    )

    xml2::xml_add_child(
      control,
      "rect",
      x = 12,
      y = 12,
      width = 92,
      height = 28,
      rx = 6,
      ry = 6,
      fill = "#f4f4f4",
      stroke = "black",
      `stroke-width` = "1"
    )

    xml2::xml_add_child(
      control,
      "text",
      "180° drehen",
      x = 58,
      y = 31,
      fill = "black",
      `font-family` = "sans-serif",
      `font-size` = "12",
      `text-anchor` = "middle"
    )
  }

  if (isTRUE(inverted)) {
    xml2::xml_add_child(
      svg,
      "text",
      "invertiert",
      x = w - 14,
      y = 30,
      fill = "black",
      `font-family` = "sans-serif",
      `font-size` = "18",
      `font-weight` = "bold",
      `text-anchor` = "end"
    )
  }

  canvas_transform <- if (isTRUE(inverted)) {
    glue::glue("rotate(180 {center_x} {center_y})")
  } else {
    NULL
  }

  canvas <- xml2::xml_add_child(svg, "g", id = "fundus-canvas", transform = canvas_transform)

  fundus <- xml2::xml_add_child(canvas, "g", id = "fundus", transform = transform_attributes)


  xml2::xml_add_child(
    fundus,
    "path",
    id = "ora",
    d = ora_path,
    fill = "rgb(229,138,133)",
    stroke = "rgb(166,89,89)",
    `stroke-width` = "3"
  )

  xml2::xml_add_child(
    fundus,
    "circle",
    id = "border",
    cx = "200",
    cy = "200",
    r = "190",
    class = "border"
  )

  xml2::xml_add_child(
    fundus,
    "circle",
    id = "equator",
    cx = "200",
    cy = "200",
    r = "95",
    class = "grid",
    `stroke-dasharray` = "4,2"
  )

  xml2::xml_add_child(
    fundus,
    "circle",
    id = "ora-ring",
    cx = "200",
    cy = "200",
    r = "142",
    class = "grid",
    `stroke-dasharray` = "2,2"
  )

  meridians <- list(
    c(254.6, 168.5, 364.5, 105),
    c(231.5, 145.4, 295, 35.5),
    c(200, 137, 200, 10),
    c(168.5, 145.4, 105, 35.5),
    c(145.4, 168.5, 35.5, 105),
    c(137, 200, 10, 200),
    c(145.4, 231.5, 35.5, 295),
    c(168.5, 254.6, 105, 364.5),
    c(200, 263, 200, 390),
    c(231.5, 254.6, 295, 364.5),
    c(254.6, 231.5, 364.5, 295),
    c(263, 200, 390, 200)
  )

  g_mer <- xml2::xml_add_child(fundus, "g", id = "meridians", class = "grid")

  for (m in meridians) {
    d <- sprintf(
      "M%s,%sL%s,%s",
      m[1], m[2], m[3], m[4]
    )

    xml2::xml_add_child(g_mer, "path", d = d)
  }

  xml2::xml_add_child(
    fundus,
    "circle",
    id = "papilla",
    cx = "245",
    cy = "195",
    r = "9",
    fill = "#fff2cc",
    stroke = "black"
  )

  macula <- xml2::xml_add_child(
    fundus,
    "g",
    id = "macula",
    transform = "translate(111.75 62.64) scale(0.65)"
  )

  xml2::xml_add_child(
    macula,
    "circle",
    cx = "140",
    cy = "200",
    r = "10",
    fill = "#c48763"
  )

  clip_url <- ifelse(clip, "url(#oraClip)", "")

  lesions <- xml2::xml_add_child(canvas,
    "g",
    id = "lesions",
    transform = glue::glue("scale({scale_image} {scale_image})"),
    `clip-path` = clip_url
  )

  xml2::xml_add_child(lesions,
                      "g",
                      id = "detachments",
                      opacity = 0.5
                      )

  vessels <- xml2::xml_add_child(
    fundus,
    "g",
    id = "vessels",
    class = "vessel"
  )

  xml2::xml_add_child(vessels, "path", d = "m 243.456,190.807 c 0,-15.69 -2.90439,-28.11473 -21.7615,-35.86911 -8.603,-0.615 -13.09989,-4.67139 -21.80289,-4.67139 -8.491,0 -22.65354,-0.50484 -30.98454,-2.69384 -0.563,-0.148 -23.74603,2.01954 -23.26003,1.43954")
  xml2::xml_add_child(vessels, "path", d = "m 245.031,191.115 c 0,-6.237 7.65007,-18.44785 10.53007,-24.42785 10.91727,-13.24988 14.66797,-9.84204 36.69793,-11.77915 3.582,0.5 8.424,0.604 11.787,1.714")
  xml2::xml_add_child(vessels, "path", d = "m 245.178,194.673 c 0,9.512 2.01727,28.48496 -3.71873,36.55796 -3.877,5.456 -11.33058,9.64469 -17.23958,11.54169 -15.103,4.847 -30.86315,2.99731 -46.73815,1.66031 -7.43,-0.625 -33.99815,-9.40554 -41.75615,-9.44054")
  xml2::xml_add_child(vessels, "path", d = "m 246.458,195.801 c 0,10.419 -0.843,25.55 3.799,35.237 6.548,13.662 19.226,20.62 33.83,21.868 4.086,0.35 9.002,0.873 12.904,-0.765 2.767,-1.162 4.397,-3.348 6.824,-5.056")

  svg
}
